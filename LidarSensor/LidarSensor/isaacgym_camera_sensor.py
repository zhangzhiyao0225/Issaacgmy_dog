...
class BaseSensor_Camera(BaseSensor):
...
class IsaacGymCameraSensor(BaseSensor_Camera):
    """
    ...
    def __init__(self, sensor_config, num_envs, gym, sim, device):
        # env, env_cfg and num_sensor are LidarSensor base class variables, not used by CameraSensor
        ...
        self.gym = gym
        self.sim = sim
        print("Initializing Isaac Gym Camera Sensor")
        print(f"Camera sensor config: {self.cfg.__dict__}")
    ...
    def add_sensor_to_env(self, env_id, env_handle, actor_handle):
        ...
        - None
        """
        print(f"Adding camera sensor to env {env_handle} and actor {actor_handle}")
        if len(self.cam_handles) == env_id:
            self.depth_tensors.append([])
            self.segmentation_tensors.append([])
            self.color_tensors.append([])
        self.cam_handle = self.gym.create_camera_sensor(
            env_handle, self.camera_properties
        )
        ...
        print(f"Camera sensor added to env {env_handle} and actor {actor_handle}")
    ...
    def capture(self):
        ...
        for env_id in range(self.num_envs):
            for cam_id in range(self.cfg.num_sensors):
                ...
                if self.cfg.segmentation_camera:
                    self.segmentation_pixels[env_id, cam_id] = (
                        self.segmentation_tensors[env_id][cam_id]
                    )
    ...
    def normalize_observation(self):
        ...
        if self.cfg.pointcloud_in_world_frame == True:
            print("ERROR: Pointcloud is in world frame. Not supported for this sensor")
        ...
        if self.cfg.sensor_noise.enable_sensor_noise == True:
            ...
            self.pixels[
                torch.bernoulli(
                    ...
                    * self.cfg.sensor_noise.pixel_dropout_prob
                )
                > 0
