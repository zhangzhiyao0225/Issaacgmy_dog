...
class LeggedRobotCfg(BaseConfig):
    class env:
        ...
        step_type = None  # trot, pace, bound

    class env_init_info:
        smoothness = False
        disturbance_force = False
        feet_pos = False
        feet_vel = False
        feet_height = False
        last_contacts  = True
        last_body_info = False

    class camera:
        use_camera = False
        camera_res = [16, 16]
        isaac_camera_type = "d"  # rgb
    ...
    class terrain:
        ...
        measure_feet_heights = False
        measure_feet_info = False
        ...
        num_point_x = len(measured_points_x)
        num_point_y = len(measured_points_y)
        max_difficulty = False
        add_roughness = False

        num_points = len(measured_points_x) * len(measured_points_y)
        selected = False  # select a unique terrains type and pass all arguments
        terrain_kwargs = None  # Dict of arguments for selected terrains
        flat_wall = None
        max_init_terrain_level = 5  # starting curriculum state

        terrain_length = 8.
        terrain_width = 8.
        num_rows = 10  # number of terrains rows (levels)
        num_cols = 20  # number of terrains cols (types)
        # terrains types: [smooth slope, rough slope, stairs up, stairs down, discrete]
        ...
        max_error_camera = None
        y_range = None
        edge_width_thresh = None

        height = None
        simplify_grid = False
        gap_size = None
        stepping_stone_distance = None
    ...
    class rewards:
        class scales:
            ...
            feet_center = 0.0
            feet_first_center = 0.0
            feet_second_center = 0.0
            feet_third_center = 0.0
        ...
        reward_type = None  # "heightfield" # none, plane, heightfield or trimesh
        measure_feet_center_reward = False

        gait_force_sigma = None
        gait_vel_sigma = None
    ...
    class privInfo:
        ...
        enableFootHeight = False

        enableLimb_mass = False

class LeggedRobotCfgPPO(BaseConfig):
    seed = 1
    runner_class_name = 'MGDPPolicyRunner'
