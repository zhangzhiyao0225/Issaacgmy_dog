...
class MjLidarSensor:
    ...
    def set_sensor_pose(self, sensor_pose):
        assert sensor_pose.shape == (4,
                                     4) and sensor_pose.dtype == np.float32, f"sensor_pose must be a 4x4 numpy array, but got {sensor_pose.shape} and {sensor_pose.dtype}"
    ...
    @ti.func
    def ray_plane_intersection(self, ray_start: ti.math.vec3, ray_direction: ti.math.vec3, center: ti.math.vec3, size: ti.math.vec3,
                               rotation: ti.math.mat3) -> ti.math.vec4:
        """计算射线与平面的交点"""
    ...
    @ti.func
    def ray_sphere_intersection(self, ray_start: ti.math.vec3, ray_direction: ti.math.vec3, center: ti.math.vec3, size: ti.math.vec3,
                                rotation: ti.math.mat3) -> ti.math.vec4:
        """计算射线与球体的交点"""
        # 返回格式: vec4(hit_x, hit_y, hit_z, t)，t为距离，t<0表示未击中
    ...
    @ti.func
    def ray_box_intersection(self, ray_start: ti.math.vec3, ray_direction: ti.math.vec3, center: ti.math.vec3, size: ti.math.vec3,
                             rotation: ti.math.mat3) -> ti.math.vec4:
        """计算射线与盒子的交点"""
        # 返回格式: vec4(hit_x, hit_y, hit_z, t)，t为距离，t<0表示未击中
    ...
    @ti.func
    def ray_cylinder_intersection(self, ray_start: ti.math.vec3, ray_direction: ti.math.vec3, center: ti.math.vec3, size: ti.math.vec3,
                                  rotation: ti.math.mat3) -> ti.math.vec4:
        """计算射线与圆柱体的交点"""
        # 返回格式: vec4(hit_x, hit_y, hit_z, t)，t为距离，t<0表示未击中
    ...
    @ti.func
    def ray_ellipsoid_intersection(self, ray_start: ti.math.vec3, ray_direction: ti.math.vec3, center: ti.math.vec3, size: ti.math.vec3,
                                   rotation: ti.math.mat3) -> ti.math.vec4:
        """计算射线与椭球体的交点"""
        # 返回格式: vec4(hit_x, hit_y, hit_z, t)，t为距离，t<0表示未击中
    ...
    @ti.func
    def ray_capsule_intersection(self, ray_start: ti.math.vec3, ray_direction: ti.math.vec3, center: ti.math.vec3, size: ti.math.vec3,
                                 rotation: ti.math.mat3) -> ti.math.vec4:
        """计算射线与胶囊体的交点"""
        # 返回格式: vec4(hit_x, hit_y, hit_z, t)，t为距离，t<0表示未击中
...
if __name__ == "__main__":
    ...
    if not args.skip_test and args.profiling:
        ...
        print("-" * 140)
        print(
            f"{'射线数量':^10} | {'传感器位姿转换':^15} | {'内存分配':^12} | {'光线数据更新':^15} | {'几何体更新':^15} | {'同步操作':^12} | {'总准备时间':^15}")
        print("-" * 140)
        for i, count in enumerate(ray_counts):
            print(
                f"{count:^10} | {sensor_pose_times[i]:^15.2f} | {memory_alloc_times[i]:^12.2f} | {rays_update_times[i]:^15.2f} | {update_geom_times[i]:^15.2f} | {sync_times[i]:^12.2f} | {prepare_times[i]:^15.2f}")
        print("-" * 140)
